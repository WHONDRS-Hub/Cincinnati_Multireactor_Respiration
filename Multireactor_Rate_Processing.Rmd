...
unique.incubations = unique(data$Sample_Name)
  ...
  colnames(rate) = c("Sample_Name","b", "DO_mg_per_L_max", "p_value_a", "p_value_b", "total_incubation_time_min", "R_Squared")
    ...
    data_site_subset = subset(data, data$Sample_Name == unique.incubations[i])
...
for (j in 1:length(unique.incubations)){
  ...
  colnames(rate) = c("Sample_Name", "linear_slope", "linear_p_value", "DO_mg_per_L_max","b", "p_value_a", "p_value_b", "total_incubation_time_min", "linear_AIC", "non_linear_AIC")
    ...
    rate$b[1] = round(as.numeric((coef_vals[2])), 3)
    ...
    geom_line(aes(y = Fitted, color = 'Non-Linear')) +
     geom_label(aes(x = 30, y = max(DO_mg_per_L + 0.5)), size = 2.5, hjust = 0,
      label = 
        paste("Linear Slope = ", fit_linear$coefficients[[2]],
      " Linear AIC = ", paste(aictab(cand.set = model_linear, modnames = mod.names.linear)$AICc), 
      "\nNon Linear B = ", as.numeric((coef_vals[2])), 
      " Non-Linear AIC = ", aictab(cand.set = model_nonlinear, modnames = mod.names.nonlinear)$AICc)) + 
