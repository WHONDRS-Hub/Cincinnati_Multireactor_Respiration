...
input.path = "C:/Github/Cincinnati/Data/RS_INC_Raw_DO_By_Min_ReadyForBoye_2024-08-28.csv"


output.path = "C:/Github/Cincinnati/Data/"
  ...
  for (j in 1:length(unique.incubations)){
    ...
    if (plot.toggle == TRUE) {
      final <- ggplot(data_site_subset_fin, aes(x = Elapsed_Minute, y = DO_mg_per_L)) + coord_cartesian(ylim = c(min(data_site_subset_fin$DO_mg_per_L)-1,max(data_site_subset_fin$DO_mg_per_L)+1))+ 
        ...
        geom_label(aes(x = 60, y = max(DO_mg_per_L + 0.5)), size = 2.5, hjust = 0,
...
write.csv(boye, paste0("C:/GitHub/Cincinnati_Multireactor_Respiration/Data/RS_Sediment_Incubations_Respiration_Rates_ReadyForBoye_",Sys.Date(),".csv"), row.names = F)
...
hist = ggplot(boye, aes(x = abs(as.numeric(Respiration_Rate_mg_DO_per_kg_per_H)))) +
  geom_histogram()
...
eca = read.csv("C:/GitHub/ECA_Multireactor_Incubations/Data/EC_Sediment_SpC_pH_Temp_Respiration.csv", skip = 2) %>% 
  ...
  filter(Respiration_Rate_mg_DO_per_kg_per_H > -200)

eca_hist = ggplot(eca, aes(x = abs(as.numeric(Respiration_Rate_mg_DO_per_kg_per_H)))) +
  geom_histogram()

combine_hist = ggplot() +
  geom_histogram(eca,binwidth = 1, mapping = aes(x = abs(as.numeric(Respiration_Rate_mg_DO_per_kg_per_H)))) +
  geom_histogram(boye, binwidth = 1, fill = "red", mapping = aes(x = abs(as.numeric(Respiration_Rate_mg_DO_per_kg_per_H))))
...
# Define the function to fit the nls model and generate the plot
fit_and_plot <- function(data) {
  sample_name <- unique(data$Sample_Name)
  
  # Define the nls model with starting parameters
  nls_model <- nls(DO_mg_per_L ~ a * exp(b * Elapsed_Minute), data = data, start = list(a = 8.5, b = -0.01))
  
  # Extract the coefficients from the model
  coef_vals <- coef(nls_model)
  
  # Create a data frame for plotting the fitted model
  fitted_data <- data
  fitted_data$Fitted = predict(nls_model, newdata = data)
  
  # Generate the plot
  p <- ggplot(data, aes(x = Elapsed_Minute, y = DO_mg_per_L)) +
...
# Apply the function to each distinct sample and collect plots
plots <- data %>%
  group_by(Sample_Name) %>%
  do(plot = fit_and_plot(.)) 

# Export the plots to a multi-page PDF
pdf("C:/GitHub/Cincinnati_Multireactor_Respiration/Data/Plots/Non_Linear_Fits/non_linear_plots.pdf")
...
# Make Data Frames
non_linear_fit <- as.data.frame(matrix(NA, ncol = 6, nrow =1))

colnames(non_linear_fit) = c("Sample_Name","b", "DO_mg_per_L_max", "p_value_a", "p_value_b", "total_incubation_time_min")


unique.incubations = unique(data$Sample_Name)

  for (j in 1:length(unique.incubations)){
    
    rate = as.data.frame(matrix(NA, ncol = 6, nrow = length(unique(data$Sample_Name))))
  
  colnames(rate) = c("Sample_Name","b", "DO_mg_per_L_max", "p_value_a", "p_value_b", "total_incubation_time_min")
    
    # Subset by individual sites
    
    data_site_subset = subset(data, data$Sample_Name == unique.incubations[j])
    ...
    fit_all = nls(DO_mg_per_L ~ a * exp(b * Elapsed_Minute), data = data_site_subset, start = list(a = max(data_site_subset$DO_mg_per_L), b = -0.01))
    ...
    rate$Sample_Name[j] = as.character(data_site_subset_fit$Sample_Name[1])
    rate$b[j] = as.numeric((coef_vals[2])) #in 1/ min
    rate$DO_mg_per_L_max[j] = round(as.numeric((coef_vals[1])), 3) #in mg O2/L
    rate$p_value_a[j] = p_value_a
    rate$p_value_b[j] = p_value_b
    
    rate$total_incubation_time_min[j] = as.numeric(tail(data_site_subset_fit$Elapsed_Minute, n = 1) - head(data_site_subset_fit$Elapsed_Minute, n = 1)) 
    
    
  rate = rate[!is.na(rate$Sample_Name),]   
  # Combining the regression metrics across locations and unique incubations
 non_linear_fit = rbind(non_linear_fit,rate)
