...
boye <- all.samples %>% 
 ...
 # mutate(rate_mg_per_L_per_h = if_else(slope_of_the_regression>=0,0,rate_mg_per_L_per_h)) %>% #make positive slopes 0
  ...
  mutate(Respiration_Rate_mg_DO_per_L_per_H = slope_of_the_regression*60) %>% # make rates positice 
...
unique.incubations = unique(samples$Sample_Name)
  ...
  for (j in 1:length(unique.incubations)){
    ...
    rate = as.data.frame(matrix(NA, ncol = 7, nrow = length(unique(samples$Sample_Name))))
...
```
```{r compare linear vs. non linear}
